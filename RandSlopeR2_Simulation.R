...
source("ADDPATH\\RandSlopeR2_PlottingFunctions.R")
source("ADDPATH\\RandSlopeR2_SimulationFunctions.R")
...
nobs     = nind*nrep
settings = c(nind=nind, nrep=nrep, alpha=alpha, betaX=betaX, betaX2=betaX2, meanX=meanX, meanX2=meanX2, Vx=Vx, Vx2=Vx2, Vr=Vr, Vu=Vu, Vv=Vv, Cuv=Cuv, randSlopeOmitted=randSlopeOmitted, errX=errX, balanced=balanced, xdist=xdist)

# Sim calculations
condRsim    = varDecomp(vect=settings)
allsim      = c(settings, condRsim)
...
plot((allests[finalOrd]-as.numeric(allsim[finalOrd]))/abs(as.numeric(allsim[finalOrd])), jitter(1:length(finalOrd)), ylab="", type="n", yaxt="n", ylim=c(1,length(finalOrd)), xlab="", xlim=c(-1, 1))
title(xlab="Proportional difference to simulated values\n(Est-Sim)/Sim", line=3.5)
axis(2, at=1:length(finalOrd), labels=ylabs, las=1)
...
# Figure 3-4 (depending on scenarios)
windows(7,7)
par(mfrow=c(1,1))
plotFig2c(resEst, allsim, main=paste0("Scenario ", scenario))
